   ...
   */
    def descendingByRetweet: TweetList = {
      if (this.isInstanceOf[Empty])
        ...
        val afterRemoving = this.remove(most)
        new Cons(most, afterRemoving.descendingByRetweet)
      }
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def filterAcc(p: Tweet => Boolean, acc: TweetSet): TweetSet = {
    if (p(elem))
      ...
      left.filter(p).filterAcc(p, right.filter(p))
  }
...
//      new NonEmpty(elem, left.filter(p), right.filter(p))
//    else
//      left.filter(p).filterAcc(p, right)
//  }

  override def union(that: TweetSet): TweetSet = {
    // That cause StackOverflowException but I don't know why
//    that.incl(elem)
//      .union(left)
//      .union(right)
    right
      .union(left.union(that.incl(elem)))
  }

  override def mostRetweeted: Tweet = {
    val leftMost = if (left.isInstanceOf[Empty]) this.elem else left.mostRetweeted
    val rightMost = if (right.isInstanceOf[Empty]) this.elem else right.mostRetweeted
