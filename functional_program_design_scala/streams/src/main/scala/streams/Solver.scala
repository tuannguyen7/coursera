...
trait Solver extends GameDef:
  ...
  def neighborsWithHistory(b: Block, history: List[Move]): LazyList[(Block, List[Move])] = {
    val neighborsList = b.legalNeighbors.map((nextB, move) => (nextB, history.appended(move)))
    LazyList.from(neighborsList)
  }
  ...
  def from(initial: LazyList[(Block, List[Move])],
    ...
    if (initial.isEmpty)
      ...
      val (headBlock, headMoves) = initial.head
      val neighbor = newNeighborsOnly(neighborsWithHistory(headBlock, headMoves), explored)
      initial.head #:: from(initial.tail #::: neighbor, explored + headBlock)
    }
  }
  ...
  lazy val pathsFromStart: LazyList[(Block, List[Move])] = {
    val start = LazyList((startBlock, List.empty))
